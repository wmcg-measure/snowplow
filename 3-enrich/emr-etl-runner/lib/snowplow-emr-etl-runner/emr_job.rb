...
module Snowplow
  module EmrEtlRunner
    class EmrJob
      ...
      Contract Bool, Bool, Bool, Bool, Bool, ConfigHash, ArrayOf[String], String, TargetsHash => EmrJob
      def initialize(debug, enrich, shred, elasticsearch, s3distcp, config, enrichments_array, resolver, targets)
        ...
        if shred
          ...
          duplicate_storage_config = self.class.build_duplicate_storage_json(targets)
    ...
    private
      ...
      Contract ArrayOf[Iglu::SelfDescribingJson] => Hash
      def self.build_duplicate_storage_json(targets)
        if targets[:DUPLICATE_TRACKING].nil?
          {}
        else
          { :duplicate_storage_config => Base64.strict_encode64(targets[:DUPLICATE_TRACKING].to_json.to_json) }
        end
